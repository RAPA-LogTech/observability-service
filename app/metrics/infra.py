...
async def get_infra_summary():
    ...
    db_pending = _amp_query_range(settings, 'db_client_connections_pending_requests', start_ts, end_ts, step_seconds)
    db_max = _amp_query_range(settings, 'db_client_connections_max', start_ts, end_ts, step_seconds)
    db_wait_p95 = _amp_query_range(settings, 'histogram_quantile(0.95, sum(rate(db_client_connections_wait_time_milliseconds_bucket[5m])) by (le))', start_ts, end_ts, step_seconds)
    db_use_p95 = _amp_query_range(settings, 'histogram_quantile(0.95, sum(rate(db_client_connections_use_time_milliseconds_bucket[5m])) by (le))', start_ts, end_ts, step_seconds)
    return {
